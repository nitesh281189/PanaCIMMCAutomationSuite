...
public static Object[][] getTestData(String testName, Xls_Reader xls) throws Exception{
	
    //Find the row number of the Testcase
	//System.out.println("In Data Util ............." + testName + xls.path);
	int testStartRowNum=1;
	//System.out.println("In Data Util ............." + xls.getCellData(Constants.DATA_SHEET,0,testStartRowNum));
	while(!xls.getCellData(Constants.DATA_SHEET,0,testStartRowNum).equals(testName))
	{
		testStartRowNum++;
		if(testStartRowNum>=1000000) 
		{
			throw new Exception("Cannot found Testcase in the Excel file!!! "+testName);
		}
	}
...
System.out.println("Total Col Number are---"+totalCols);
...
for(int rNum=dataStartRowNumber;rNum<finalrows;rNum++){
	...
	for(int cNum=0;cNum<totalCols;cNum++){//put data in Hastable
